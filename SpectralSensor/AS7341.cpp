...
AS7341::AS7341(MCP2221& i2cController)
	: m_i2c(i2cController)
	, m_readingState(AS7341_WAITING_DONE)
	, m_last_spectral_int_source(0)
	, m_basicCounts{ 0 }
{
...
bool AS7341::enableLED(bool enable_led)
{
	...
	bool result = m_i2c.modifyRegisterBit(AS7341_CONFIG, enable_led, 3)
		&& m_i2c.modifyRegisterBit(AS7341_LED, enable_led, 7);
...
void AS7341::calculateDataSensorCorrection()
{
	...
	{
		// TODO
		//m_correctedCounts[channel] = correctionFactors[channel] * (m_basicCounts[channel] - offsetCompensationValues[channel]);
...
uint8_t AS7341::getOptimizedMeasurementValues(bool checkState, bool optimizedValuesDetected, uint16_t rawVal[], double basicVal[], double corrVal[])
{
...
//		{
//			//BaseFunctions.DebugOut(true, "Noise gain: " + currentGain.ToString() + " Raw: " + rawVal.Max().ToString());
...
//		else
//		{
//			//BaseFunctions.DebugOut(true, "Ok gain: " + currentGain.ToString() + " Raw: " + rawVal.Max().ToString());
//			break;
//		}
//	}
//
